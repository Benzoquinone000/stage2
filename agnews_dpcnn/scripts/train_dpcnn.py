...
def make_loader(
    ...
    dataset = AGNewsDataset(rows, vocab, max_length)
...
def macro_f1_from_preds(preds: torch.Tensor, labels: torch.Tensor, num_classes: int = 4) -> float:
    scores = []
...
def evaluate(
    ...
    return {
        ...
        "macro_f1": macro_f1_from_preds(preds, labels),
        "logits": logits,
        "labels": labels,
    }
...
def save_checkpoint(path: Path, model: DPCNN, config: TrainConfig, vocab: dict[str, int]) -> None:
    torch.save(
        {
